...
class SqlManager {
	...
	public function get($limit = 1, $offset = 0) {
		...
		if ($limit == 1) {
			$result = MysqlAdapter::init($this->_empty_model_instance->db_name)->getOne($query_conf['query'], $query_conf['set']);
		} else {
			$result = MysqlAdapter::init($this->_empty_model_instance->db_name)->getMany($query_conf['query'], $query_conf['set']);
		}
	...
	public function filter_primary():self {
		...
		$this->_where_set[QueryBuilder::FILTER_EQUAL] = array_merge($this->_where_set[QueryBuilder::FILTER_EQUAL], $set);
